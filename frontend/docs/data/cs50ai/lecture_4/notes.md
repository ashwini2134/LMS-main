...
## [__Machine Learning__](https://cs50.harvard.edu/ai/notes/4/#machine-learning)
...
## [__Supervised Learning__](https://cs50.harvard.edu/ai/notes/4/#supervised-learning)
...
![Classification](/static/images/image_122856941538.png)

## [__Nearest\-Neighbor Classification__](https://cs50.harvard.edu/ai/notes/4/#nearest-neighbor-classification)
...
![Nearest Neighbor Classification](/static/images/image_122856940665.png)
...
One way to get around the limitations of nearest\-neighbor classification is by using __k\-nearest\-neighbors classification__, where the dot is colored based on the most frequent color of the *k* nearest neighbors\. It is up to the programmer to decide what *k* is\. Using a 3\-nearest neighbors classification, for example, the white dot above will be colored blue, which intuitively seems like a better decision\.
...
## [__Perceptron Learning__](https://cs50.harvard.edu/ai/notes/4/#perceptron-learning)
...
![Decision Boundary](/static/images/image_122856940758.png)
...
The weights and values are represented by vectors, which are sequences of numbers \(which can be stored in lists or tuples in Python\)\. We produce a Weight Vector __w__: \(w₀, w₁, w₂\), and getting to the best weight vector is the goal of the machine learning algorithm\. We also produce an Input Vector __x__: \(1, x₁, x₂\)\.
...
![Dot Product Equation](/static/images/image_122856940803.png)

Since the goal of the algorithm is to find the best weight vector, when the algorithm encounters new data it updates the current weights\. It does so using the __perceptron learning rule__:

![Perceptron Learning Rule](/static/images/image_122856941538.png)
...
![Hard Threshold](/static/images/image_122856940665.png)

The problem with this type of function is that it is unable to express uncertainty, since it can only be equal to 0 or to 1\. It employs a __hard threshold__\. A way to go around this is by using a logistic function, which employs a __soft threshold__\. A logistic function can yield a real number between 0 and 1, which will express confidence in the estimate\. The closer the value to 1, the more likely it is to rain\.

![Soft Threshold](/static/images/image_122856940758.png)

## [__Support Vector Machines__](https://cs50.harvard.edu/ai/notes/4/#support-vector-machines)
...
![Circle Decision Boundary](/static/images/image_122856941538.png)
...
## [__Regression__](https://cs50.harvard.edu/ai/notes/4/#regression)
...
![Regression](/static/images/image_122856940665.png)

## [__Loss Functions__](https://cs50.harvard.edu/ai/notes/4/#loss-functions)
...
For classification problems, we can use a __0\-1 Loss Function__\.
...
![0-1 Loss Function](/static/images/image_122856940758.png)
...
![L₁](/static/images/image_122856940803.png)

## [__Overfitting__](https://cs50.harvard.edu/ai/notes/4/#overfitting)
...
![Overfitting](/static/images/image_122856941538.png)
...
## [__Regularization__](https://cs50.harvard.edu/ai/notes/4/#regularization)
...
![Banknotes](/static/images/image_122856940665.png)
...
## [__Reinforcement Learning__](https://cs50.harvard.edu/ai/notes/4/#reinforcement-learning)
...
![Reinforcement Learning](/static/images/image_122856940758.png)
...
## [__Markov Decision Processes__](https://cs50.harvard.edu/ai/notes/4/#markov-decision-processes)

Reinforcement learning can be viewed as a Markov decision process, having the following properties:

- Set of states __*S*__
- Set of actions __*Actions\(S\)*__
- Transition model __*P\(s’ | s, a\)*__
- Reward function __*R\(s, a, s’\)*__

For example, consider the following task:

![Markov Decision Process Demo](/static/images/image_122856940803.png)
...
## [__Q\-Learning__](https://cs50.harvard.edu/ai/notes/4/#q-learning)

Q\-Learning is one model of reinforcement learning, where a function __*Q\(s, a\)*__ outputs an estimate of the value of taking action *a* in state *s*\.

The model starts with all estimated values equal to 0 \(__*Q\(s,a\)* = 0__ for all *s, a*\)\. When an action is taken and a reward is received, the function does two things: 1\) it estimates the value of __*Q\(s, a\)*__ based on current reward and expected future rewards, and 2\) updates __*Q\(s, a\)*__ to take into account both the old estimate and the new estimate\. This gives us an algorithm that is capable of improving upon its past knowledge without starting from scratch\.

__*Q\(s, a\) ⟵ Q\(s, a\) \+ α\(new value estimate \- Q\(s, a\)\)*__

The updated value of __*Q\(s, a\)*__ is equal to the previous value of __*Q\(s, a\)*__ in addition to some updating value\. This value is determined as the difference between the new value and the old value, multiplied by α, a learning coefficient\. When α = 1 the new estimate simply overwrites the old one\. When α = 0, the estimated value is never updated\. By raising and lowering α, we can determine how fast previous knowledge is being updated by new estimates\.
...
![Q Learning Formula](/static/images/image_122856941538.png)

A __Greedy Decision\-Making__ algorithm completely discounts the future estimated rewards, instead always choosing the action __*a*__ in current state __*s*__ that has the highest __*Q\(s, a\)*__\.

This brings us to discuss the __Explore vs\. Exploit__ tradeoff\. A greedy algorithm always exploits, taking the actions that are already established to bring to good outcomes\. However, it will always follow the same path to the solution, never finding a better path\. Exploration, on the other hand, means that the algorithm may use a previously unexplored route on its way to the target, allowing it to discover more efficient solutions along the way\. For example, if you listen to the same songs every single time, you know you will enjoy them, but you will never get to know new songs that you might like even more\!

To implement the concept of exploration and exploitation, we can use the __ε \(epsilon\) greedy__ algorithm\. In this type of algorithm, we set ε equal to how often we want to move randomly\. With probability 1\-ε, the algorithm chooses the best move \(exploitation\)\. With probability ε, the algorithm chooses a random move \(exploration\)\.
...
## [__k\-means Clustering__](https://cs50.harvard.edu/ai/notes/4/#k-means-clustering)
...
![k-means Clustering](/static/images/image_122856940665.png)
...
## [__Quiz 4__](https://cs50.harvard.edu/ai/quizzes/4/#quiz-4)
...
### [__Question 1__](https://cs50.harvard.edu/ai/quizzes/4/#question-1)

__Categorize the following: A social network's AI uses existing tagged photos of people to identify when those people appear in new photos\.__
...
### [__Question 2__](https://cs50.harvard.edu/ai/quizzes/4/#question-2)

__Imagine a regression AI that makes the following predictions for the following 5 data points\. What is the total L2 loss across all of these data points \(i\.e\., the sum of all the individual L2 losses for each data point\)?__
...
### [__Question 3__](https://cs50.harvard.edu/ai/quizzes/4/#question-3)

__If Hypothesis 1 has a lower L1 loss and a lower L2 loss than Hypothesis 2 on a set of training data, why might Hypothesis 2 still be a preferable hypothesis?__
...
### [__Question 4__](https://cs50.harvard.edu/ai/quizzes/4/#question-4)

__In the ε\-greedy approach to action selection in reinforcement learning, which of the following values of ε makes the approach identical to a purely greedy approach?__
...
## [__Project 4__](https://cs50.harvard.edu/ai/projects/4/#project-4)

Projects are *optional, but encouraged*\. They are a good way to apply the concepts learned in lecture to practical problems\. Consider the following project to deepen your understanding of machine learning and classification\.

### [__Machine Learning Classification Project__](https://cs50.harvard.edu/ai/projects/4/#ml-classification-project)

__Build a machine learning classifier using scikit-learn\.__

**Objective**: Create a classification system that can categorize data using various machine learning algorithms learned in this lecture.

**Requirements**:
- Implement multiple classification algorithms (SVM, Perceptron, Naive Bayes, k-NN)
- Use proper data preprocessing and feature engineering
- Implement cross-validation and performance evaluation
- Compare different algorithms and analyze their performance
...
- Refer to the lecture notes for algorithm implementation guidance
- Explore UCI Machine Learning Repository for datasets
...
<details class="cursor-pointer my-4 p-4 rounded bg-slate-800 border border-slate-700">
  ...
  <div class="mt-2 text-slate-300">
    ...
    <ul>
      <li>Start with data exploration and visualization</li>
      <li>Use proper train/test splitting and cross-validation</li>
      <li>Experiment with different preprocessing techniques</li>
      <li>Compare algorithms using appropriate metrics</li>
